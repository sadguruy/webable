...
function renderLeadingIcon({
  ...
  spritePath,
  spriteIconId,
}: Pick<ActionListsBaseProps, "icon" | "iconSrc" | "iconAlt" | "spritePath" | "spriteIconId">) {
  if (icon) {
    return icon;
  }
  ...
  if (spritePath && spriteIconId) {
    return (
      <svg className="action-lists__sprite" aria-hidden={iconAlt ? undefined : true}>
...
export function ActionLists({
  ...
  const content = (
    <>
      <span className="action-lists__left">
        {leadingIcon ? (
          <span className="action-lists__icon" aria-hidden={iconAlt ? undefined : true}>
            {leadingIcon}
          </span>
        ) : null}
        <span className="action-lists__text">
          <span className="action-lists__title">{title}</span>
          {helperText ? <span className="action-lists__helper">{helperText}</span> : null}
        </span>
      </span>
  ...
  if (elementType === "link") {
    const { href, target = "_self", rel, onClick, ...anchorProps } = props as ActionListsLinkProps;
